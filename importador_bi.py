        ...
        for index, row in df.iterrows():
            ...
            if isinstance(tempo, str):
                if "em atendimento" in tempo.lower():
                    ...
                    # Converter string "HH:MM:SS" para um formato reconhecido pelo SQL Server
                    try:
                        h, m, s = map(int, tempo.split(':'))
                        duracao = f"{h:02}:{m:02}:{s:02}"
                    except ValueError:
                        self.log.append(f"Linha {index + 2} (Protocolo {protocolo}): Erro - Formato de tempo inválido.")
                        continue
            else:
                duracao = tempo
