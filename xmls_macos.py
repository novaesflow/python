...
def generate_insert_statements_from_xml(file_path):
    ...
    for det in root.findall(".//n:det", namespaces=ns):
        ...
        cProd = prod.find("n:cProd", namespaces=ns).text.zfill(14)  # Ensure cProd has 14 digits

        # Use cProd if cEAN is 'SEM GTIN' or cEAN is not present
        if cEAN == "SEM GTIN" or not cEAN.strip():
            cEAN = cProd

        # Ensure cEAN has 14 digits
        cEAN = cEAN.zfill(14)
    ...
    return product_inserts, list(unique_tax_inserts)
...
def process_all_xmls_in_directory(directory_path):
    ...
    for xml_file in xml_files:
        product_inserts, tax_inserts = generate_insert_statements_from_xml(xml_file)
...
# Usage example
directory_path = "/Users/novaes/Desktop/Novaes/Python/XMLS"
product_sql_file, tax_sql_file = process_all_xmls_in_directory(directory_path)
