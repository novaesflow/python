...
def read_existing_procods(file_path):
    with open(file_path, 'r') as file:
        existing_procods = {line.strip().zfill(14) for line in file}  # Garantir que todos os PROCODs tenham 14 dígitos
...
def generate_insert_statements_from_xml(file_path, existing_procods):
    ...
    root = tree.getroot()
    product_inserts = []
    unique_tax_inserts = set()  # Usar um conjunto para evitar duplicidades
    ...
    for det in root.findall(".//n:det", namespaces=ns):
        ...
        cProd = prod.find("n:cProd", namespaces=ns).text.zfill(14)  # Assegurar 14 dígitos
        if cEAN == "SEM GTIN":
            cEAN = cProd
        cEAN = cEAN.zfill(14)  # Assegurar 14 dígitos para todos os códigos
        if cEAN not in existing_procods:
            existing_procods.add(cEAN)
                    ...
                    if tax_detail is not None:
                        ...
                        if cst in cst_mapping:
                            for impfedsim in cst_mapping[cst]:
                                ...
                                unique_tax_inserts.add(tax_sql)  # Adicionar ao conjunto para manter a unicidade

    return product_inserts, list(unique_tax_inserts)  # Converter o conjunto para lista antes de retornar
